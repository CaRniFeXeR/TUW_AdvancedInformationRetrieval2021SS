...
class TK(nn.Module):
    ...
    def forward(self, query: Dict[str, torch.Tensor], document: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if not self.secondary_batch_output_logger is None:
            self.secondary_batch_output_logger.log(secondary_batch_output=SecondaryBatchOutput(score=output, per_kernel=s_log, cosine_matrix=cosine_matrix_m, query_id=query["id"], doc_id=document["id"]))
